...
export async function renderMap(container, onTap) {
  ...
  // Atașăm controller-ul de zoom/pan
  mapState.zoomCtl = attachZoomPan(svg);
}
...
export function setBase(code) {
  ...
  // Auto-zoom pe bază + vecini
  if (mapState.zoomCtl && mapState.centroids[code]) {
    const [cx, cy] = mapState.centroids[code];
    setTimeout(() => mapState.zoomCtl.setView(cx, cy, 2.8), 100);
  }
}
...
export function resetView() {
  if (mapState.zoomCtl) mapState.zoomCtl.reset();
}
