// =====================================================
// IQ-TRIVIA-RO / FAZA 3A / Game Logic v4.1 (Hotfix ASCII)
// Fix: smart quotes + en-dash decrement -> safe ASCII
// Patches: P1 answer_log + P2 level matching + P4 auto-profile
// =====================================================
...
// Profil inexistent -> creeaza unul cu username din metadata sau email
const username =
...
els[‘info-panel’].textContent = ‘Tap pe un vecin (cyan) sau o insula pentru a ataca’;
return;
}
startAttack(code);
}
...
els[‘info-panel’].textContent = ‘Se incarca intrebari…’;
const lvl = computeLevel();
state.questions = await fetchQuestions(lvl);

if (state.questions.length < 3) {
els[‘info-panel’].textContent = ‘Stoc intrebari noi epuizat. Reseed DB.’;
return;
}
...
// Exclude intrebari deja raspunse (din answer_log)
...
// Range ELO real: +/- ELO_RANGE in jurul rating-ului userului
...
// Fallback 1: relax range ELO, pastreaza level
if (candidates.length < 3) {
console.warn(’[fetchQuestions] Range ELO restrans. Fallback: orice difficulty pe level’, safeLvl);
...
// Diversitate categorii – alege cate una din fiecare display_order dorit
const result = [];
...
// P1: onAnswer cu telemetrie completa in answer_log
...
// P1: scrie in answer_log cu toate campurile NOT NULL
...
els[‘question-screen’].classList.add(‘hidden’);
els[‘defeat-message’].textContent = data?.message || `Nu ai reusit sa cuceresti ${getCountryName(code)}.`;
...
clearInterval(state.timerInterval);
els[‘question-screen’].classList.add(‘hidden’);
els[‘info-panel’].textContent = ‘Ai renuntat. -5 ELO.’;
}
