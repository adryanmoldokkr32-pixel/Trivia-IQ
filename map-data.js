export const COUNTRIES_DATA = {
  ...
  CY: { name: 'Cipru', neighbors: ['TR'], cx: 750, cy: 620, isolated: true },
  HR: { name: 'Croatia', neighbors: ['SI','HU','RS','BA','ME'], cx: 480, cy: 460 },
  DK: { name: 'Danemarca', neighbors: ['DE','SE'], cx: 410, cy: 240 },
  CH: { name: 'Elvetia', neighbors: ['DE','FR','IT','AT','LI'], cx: 400, cy: 410 },
  EE: { name: 'Estonia', neighbors: ['LV','RU','FI'], cx: 620, cy: 200 },
  FI: { name: 'Finlanda', neighbors: ['SE','NO','RU','EE'], cx: 600, cy: 130 },
  FR: { name: 'Franta', neighbors: ['BE','LU','DE','CH','IT','ES','AD','MC'], cx: 320, cy: 430 },
  DE: { name: 'Germania', neighbors: ['DK','PL','CZ','AT','CH','FR','LU','BE','NL'], cx: 430, cy: 340 },
  GR: { name: 'Grecia', neighbors: ['AL','MK','BG','TR'], cx: 580, cy: 580 },
  IE: { name: 'Irlanda', neighbors: [], cx: 200, cy: 290, isolated: true },
  IS: { name: 'Islanda', neighbors: [], cx: 130, cy: 100, isolated: true },
  ...
  MT: { name: 'Malta', neighbors: [], cx: 470, cy: 620, isolated: true },
  ...
  RO: { name: 'Romania', neighbors: ['HU','UA','MD','BG','RS'], cx: 605, cy: 440 },
...
export function getNeighbors(code) {
  return COUNTRIES_DATA[code] ? COUNTRIES_DATA[code].neighbors : [];
}

export function areNeighbors(code1, code2) {
  return getNeighbors(code1).indexOf(code2) !== -1;
}

export function getCountryName(code) {
  return COUNTRIES_DATA[code] ? COUNTRIES_DATA[code].name : code;
}

export function getIsolatedCountries() {
  return COUNTRY_CODES.filter(code => COUNTRIES_DATA[code].isolated === true);
}

export function isIsolated(code) {
  return COUNTRIES_DATA[code]?.isolated === true;
}
